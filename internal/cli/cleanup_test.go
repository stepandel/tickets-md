...
func TestFindOrphanWorktreeAndBranchIDs(t *testing.T) {
	...
	runCleanupGit(t, s.Root, "branch", worktree.BranchPrefix+"TIC-888")
	...
	branchIDs, err := findOrphanBranchIDs(s)
	if err != nil {
		t.Fatalf("findOrphanBranchIDs: %v", err)
	}
	if len(branchIDs) != 1 || branchIDs[0] != worktree.BranchPrefix+"TIC-888" {
		t.Fatalf("branch IDs = %v, want [%sTIC-888]", branchIDs, worktree.BranchPrefix)
	}
}
...
func TestCollectConfiguredStageActions(t *testing.T) {
	...
	want := []string{
		...
		"delete branch " + worktree.BranchPrefix + done.ID + " for " + done.ID + " in done",
	}
...
func TestCollectConfiguredStageActionsSkipsActiveRuns(t *testing.T) {
	...
	if _, err := worktree.Create(s.Root, tk.ID, ""); err != nil {
		t.Fatalf("Create worktree: %v", err)
	}
...
func TestExecuteCleanupActionsDryRunMutatesNothing(t *testing.T) {
	...
	if !worktreeExists(s.Root, tk.ID) {
	...
	if out := runCleanupGit(t, s.Root, "branch", "--list", worktree.BranchPrefix+tk.ID); out == "" {
