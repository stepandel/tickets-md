...
- A board view (`Open: Tickets Board` command) that mirrors the
  ...
  tickets whose id, title, priority, project, stage, agent status,
  or linked ticket ids match the query. Queries are tokenized on
  whitespace, each token matches case-insensitively by substring, and
  all tokens must match somewhere across those fields. The query is
  persisted per board leaf across reopen/reload.
...
- A board-level filter input in the board header that narrows the
  visible cards by id, title, priority, project, stage, agent
  status, and linked ticket ids. Queries are tokenized on
