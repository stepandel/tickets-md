...
- The Obsidian board view now has a filter input in its header that
  narrows visible cards using case-insensitive matching across ticket
  id, title, priority, project, stage, and agent status.
  Space-separated query tokens are combined with AND semantics, so
  every token must match somewhere across those fields. The query is
  persisted per board leaf across reopen/reload, and empty stages
